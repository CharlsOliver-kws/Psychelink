        ...
        Mono.fromRunnable(() -> {
                    try {
                        ...
                        if (riskLevel == ChatMessage.RiskLevel.HIGH || riskLevel == ChatMessage.RiskLevel.MEDIUM) {
                            emailSent = mcpEmailService.sendAlertEmail(username, message);
                        }
                        ...
                        // 邮件失败已在 sendAlertEmail 内部消化；这里兜底事件落库等异常，避免静默丢失
                        log.error("风险事件异步处理失败: user={}, risk={}", username, riskLevel, e);
                    }
                })
                .subscribeOn(Schedulers.boundedElastic())
                .subscribe();
